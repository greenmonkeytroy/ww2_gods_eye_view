...
test('the voice TOOL SCHEMA is byte-identical to main — the mission mapping is instructions only', () => {
  ...
  // Re-pinned 2026-09-21: the WW2 Naval Battles layer DELIBERATELY adds
  // 'local-ww2-naval' to set_layer_visibility/show_data_layers_menu/
  // get_entity_context's enums, and the WW2 Country Labels layer adds
  // 'ww2-country-labels' to the first two, each with its common-name
  // mapping in the description (real new data layers — exactly the kind of
  // schema change this pin exists to make loud). The guarded claim is
  // unchanged: first-run missions ride existing tools, and any NEW drift
  // from this recorded schema still fails here.
  assert.equal(block.length, 31461, 'tool schema byte length drifted from the pinned release schema');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    'ebb6c27d7de4ba74364ee0a23be44fe9c62bfca15a626685c9d7ad4fca5610c0',
